...
/*   Updated: 2019/10/09 23:00:28 by hfrankly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		ft_get_ants(t_sdl *sdl)
{
	...
	while (++i < sdl->arrsize)
	{
		...
		while (sdl->cmdline[i][offset] != '-')
			offset++;
		offset++;
		sdl->ants[i].dstroom = sdl->farm->
		h_tab[hash_func(&sdl->cmdline[i][offset], sdl->farm->size)].room;
		sdl->ants[i].x = sdl->ants[i].srcroom->x;
		sdl->ants[i].y = sdl->ants[i].srcroom->y;
		sdl->ants[i].radius = 10;
		sdl->ants[i].color = ((sdl->ants[i].antnbr + 1) *
		0x0011AAEE) | 0xFF000000;
	}
}
